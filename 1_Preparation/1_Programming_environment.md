...
With new versions being released, software changes its behavior. Even though the changes are usually improvements and desired, it can make reproducing someone else's work a lot harder if you don't have access to the exact same software that was used in the analysis. Furthermore, if you update a tool for a project, this might prevent the code in another project from running smoothly, as the latter might depend on the old functionality of the software.
